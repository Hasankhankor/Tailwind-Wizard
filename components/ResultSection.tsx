  ...
  return (
    <section className='font-[Consolas,_"Courier_New",_monospace] lgx:col-start-2 relative lgx:h-full max-lgx:h-1/2 overflow-y-auto text-[#111827] dark:text-[#abb2bf]'>
      <div className="absolute right-[16px] top-[16px] flex items-center">
        ...
        <a target="_blank" href="https://github.com/Hasankhankor/Tailwind-Wizard" rel="noreferrer">
          ...
          {computedResultVals.map((it) => (
            <div key={it.id}>
              ...
              <p className="text-[18px] leading-[30px] mb-[16px] mt-[8px]">
                ...
                <FlipMove
                  className="dark:bg-[#1e1e1e] bg-[#e8e8e8] dark:text-[#6159cf] text-[#221c72] rounded-[2px] pt-[6px] pr-[10px] pl-[2px] inline-flex flex-wrap"
                  typeName="span"
                ...
                <button
                  className="bg-gradient-to-r from-blue-500 to-blue-800 text-white font-semibold py-2 px-4 rounded-lg shadow-lg transition-transform transform hover:scale-105 hover:shadow-xl active:scale-95 active:shadow-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                  onClick={() => {
                    copyText(it.resultVal.map(v => v.val).join(' '))
                  }}
                >
                  Copy {it.selectorName}
                </button>
